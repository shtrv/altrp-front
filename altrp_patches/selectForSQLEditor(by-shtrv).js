...
function convertShortcodes(sql, qs) {
    ...
    while (true) {
        ...
        if (code.indexOf('IF_OR_REQUEST') !== -1) {
            _sql = _sql.replace(shortCode, replaceIfOrRequest(code, qs));
        }
...
function replaceIfAndRequest(code, qs) {
    let query = ``;
    let args = code.split(':');
...
exports.convertShortcodes = convertShortcodes;
function replaceIfOrRequest(code, qs) {
    let query = ``;
    let args = code.split(':');
    let arg1 = args[1].trim();
    let arg2 = args[2].trim();
    ...
    if (qs[arg2] && typeof qs[arg2] !== 'undefined') {
        ...
        switch (arg3) {
            ...
            case 'NOT_IN':
                ...
                query = ` OR ${arg1} ${arg3 == 'IN' ? arg3 : 'NOT IN'} (${searchable}) `;
                break;
            case 'LIKE':
            case 'ILIKE':
                searchable = `'%${qs[arg2]}%'`;
                query = ` OR ${arg1} ${arg3} ${searchable} `;
                break;
            case 'START_LIKE':
                searchable = `'%${qs[arg2]}'`;
                query = ` OR ${arg1} LIKE ${searchable} `;
                break;
            case 'END_LIKE':
                searchable = `'${qs[arg2]}%'`;
                query = ` OR ${arg1} LIKE ${searchable} `;
...
exports.replaceIfOrRequest = replaceIfOrRequest;
function replaceIsNull(code, qs) {
    let args = code.split(':');
    let arg1 = args[1].trim();
    return typeof qs[arg1] !== 'undefined' ? ` false ` : ` true `;
